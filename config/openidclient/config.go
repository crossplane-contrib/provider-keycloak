...
func Configure(p *config.Provider) {
	...
	p.AddResourceConfigurator("keycloak_openid_client_admin_permissions", func(r *config.Resource) {
		r.ShortGroup = Group

		// client_ids holds the UUIDs of the clients this permission applies
		// to. Those can be OpenID as well as SAML clients, so expose one
		// strongly-typed list field per client type. The original client_ids
		// field stays settable for raw IDs.
		multitypes.ApplyToAsListWithOptions(r, "client_ids",
	...
	p.AddResourceConfigurator("keycloak_openid_client_authorization_permission", func(r *config.Resource) {
		...
		// policies before they are sent to Terraform. The original policies
		// field stays settable for raw IDs of policy types that have no
		// managed resource yet.
		multitypes.ApplyToAsListWithOptions(r, "policies",
			&multitypes.Options{KeepOriginalField: true},
			multitypes.Instance{
				Name: "policies",
