...
func TestOpenidClientAdminPermissionsReferences(t *testing.T) {
	...
	for flavourName, get := range flavours {
		t.Run(flavourName, func(t *testing.T) {
			...
			r, ok := p.Resources["keycloak_openid_client_admin_permissions"]
			if !ok {
				t.Fatalf("keycloak_openid_client_admin_permissions: resource not registered in provider")
			}
			...
			for _, field := range []string{"policies", "client_ids"} {
				s, ok := r.TerraformResource.Schema[field]
				if !ok {
					t.Fatalf("%s: field missing from schema", field)
				}
				if s.Computed {
					t.Errorf("%s: field is computed, expected it to stay settable", field)
				}
			}
		})
	}
