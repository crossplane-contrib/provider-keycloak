...
func TestRoleAdminPermissionsReferences(t *testing.T) {
	...
	for flavourName, get := range flavours {
		t.Run(flavourName, func(t *testing.T) {
			...
			r, ok := p.Resources["keycloak_role_admin_permissions"]
			if !ok {
				t.Fatalf("keycloak_role_admin_permissions: resource not registered in provider")
			}
