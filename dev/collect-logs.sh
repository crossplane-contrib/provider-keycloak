...
kubectl logs statefulset/keycloak-keycloakx -n keycloak > logs/keycloak.yaml
kubectl logs deployment/openldap-deployment -n keycloak > logs/openldap.yaml
kubectl logs deployment/crossplane -n crossplane-system crossplane > logs/crossplane.yaml
kubectl logs deployment/crossplane -n crossplane-system dev > logs/crossplane-dev.yaml
kubectl logs deployment/crossplane -n crossplane-system crossplane-init > logs/crossplane-init.yaml
provider=$(kubectl get deployment -n crossplane-system -o jsonpath='{range .items[*]}{.metadata.name}{"\n"}{end}'| grep keycloak-)
kubectl logs deployment/${provider} -n crossplane-system > logs/crossplane-keycloak-provider.yaml
