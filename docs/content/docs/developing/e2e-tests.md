...
Hand-written controllers (ones not generated by upjet, such as
`connectionsecrettransform`) have their own test suite that runs **Kyverno
chainsaw directly** rather than through uptest. This makes individual test
steps explicit and allows infrastructure like Envoy Gateway and Traefik to be
deployed as real OIDC consumers that actually validate the secrets the
controllers produce.
...
| Path | Purpose |
|------|---------|
| `cluster/test/controller/` | Root directory for chainsaw-based controller tests |
| `cluster/test/controller/setup.sh` | Installs infrastructure (Envoy Gateway, Traefik) before chainsaw runs |
| `cluster/test/controller/<name>/chainsaw-test.yaml` | One chainsaw `Test` per controller under test |

### Running locally

```bash
# 1. Start the kind cluster with the provider and Keycloak
./dev/setup_dev_environment.sh --direct-helm

# 2. Install Envoy Gateway + Traefik and run the chainsaw tests
make controller-e2e
```
...
```bash
./cluster/test/controller/setup.sh
.cache/tools/*/chainsaw-* test --test-dir cluster/test/controller --namespace dev
```

### CI job

The `e2e-controller-tests` CI job runs on every PR. It reuses the same
`fenrir-1` kind cluster pattern as the regular `e2e-tests` job: the provider
is deployed via `dev/setup_dev_environment.sh --deploy-local-provider`, then
`cluster/test/controller/setup.sh` installs the infrastructure and
`make controller-e2e` runs chainsaw.

### Writing a new controller test

1. Create `cluster/test/controller/<controller-name>/chainsaw-test.yaml`.
2. Each chainsaw `step` uses `apply` (create resources), `assert` (wait for
   state), `error` (assert absence), and `delete` (explicit cleanup).
3. If the test needs new infrastructure (e.g. a third-party OIDC consumer),
   add its installation to `cluster/test/controller/setup.sh`.
4. Provider-managed resources (Realms, Clients, etc.) that the test creates
   are cleaned up by chainsaw's `cleanup` block; the chainsaw test does not
   need to register them in `cluster/test/cases.txt` since they are not part
   of the uptest demo suite.
