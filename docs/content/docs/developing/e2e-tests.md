...
CI does not always run every demo. `scripts/e2e_dag.py select` classifies the
changed files of a pull request into a tier:

| Tier | Trigger | Scope |
|------|---------|-------|
| `full` | `go.mod`, `go.sum`, `Makefile`, `internal/`, `cmd/`, `build/`, `ci.yml`, or any non-PR event | all demos × all Keycloak versions |
