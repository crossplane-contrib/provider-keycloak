...
description: How to write and debug end-to-end tests for provider-keycloak
weight: 5
---
...
2. the touched API groups and which changed paths implied them;
3. every selected demo with the reason it is in the set — `uses API group 'x'`,
   `changed directly`, `prerequisite of <demo>` or `depends on <demo>`.
