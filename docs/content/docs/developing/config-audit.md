...
A `multitype` class is a classification of the attribute, not a work item: it
names no resource to change, and the resource missing a family member is
reported per resource by `missing-multitype`. It is therefore never counted as
actionable. Its status is decided within its own shape group, so a required and
an optional attribute of the same name never decide each other's status.
...
Only actionable findings are filed: satisfied, `protocolSpecific` and
`multitype`-class ones are skipped, and `unclassified` is off by default (`--audit-detectors` selects the
