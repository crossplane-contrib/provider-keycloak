...
For a field whose **type** changed between two versions, use the shared helper
in [`config/conversion`](config/conversion/conversion.go), which freezes the
current version, promotes the new one to storage/hub and registers both the
upgrade and the downgrade conversion:
...
* Types of previous versions live in `apis/**/<previous-version>/` and are **not**
  regenerated by upjet — upjet only generates types for `r.Version`. `generate/generate.go`
  therefore only deletes the generated conversion hubs/spokes and resolvers, never the
  whole `zz_*` tree.
* Every CRD that serves more than one version needs `spec.conversion.strategy: Webhook`.
  `controller-gen` does not emit it, and Crossplane's package manager only injects the
  `caBundle` and the service coordinates into a conversion stanza that already exists.
  `cmd/crdconversion` runs as part of `make generate` and adds the stanza to all
  multi-version CRDs.
* The conversion webhook is only served when the provider is started with a certificates
  directory (`--certs-dir` / `TLS_SERVER_CERTS_DIR`), which Crossplane provides as long as
  `webhooks.enabled` is `true` (the default) in the Crossplane installation.
...
Unit tests for the registered conversions live in `config/conversion_test.go`. They run the
...
very same object through two different API versions. Run them against a cluster that already
has the provider deployed:
